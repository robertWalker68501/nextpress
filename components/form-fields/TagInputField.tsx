...
export function TagInput({
  ...
  const addTags = React.useCallback(
    (rawValue: string) => {
       ...
       */
      const candidates = rawValue.split(',').map(normalizeTag).filter(Boolean);
  ...
  return (
    <div className={cn('space-y-2', className)}>
      <div
        className={cn(
          'border-input flex min-h-10 flex-wrap items-center gap-2 rounded-md border bg-transparent px-3 py-2 shadow-xs transition-[color,box-shadow]',
        ...
        {value.map((tag) => (
          ...
          >
            ...
            <button
              ...
              className={cn(
                'text-muted-foreground rounded-sm transition-colors',
