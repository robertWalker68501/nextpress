...
import { editorPresets, type EditorPreset } from './editor-presets';

type TinyMceInit = NonNullable<ComponentProps<typeof Editor>['init']>;

type ImagesUploadHandler = NonNullable<TinyMceInit['images_upload_handler']>;
...
function readShadcnTheme(isDark: boolean): ShadcnEditorTheme {
  ...
  return {
    ...
    cardForeground: cssVar(root, '--card-foreground', fallback.cardForeground),
    ...
    fontSans:
      cssVar(root, '--font-sans', '') || body.fontFamily || fallback.fontSans,
    fontMono: cssVar(root, '--font-mono', fallback.fontMono),
  };
}
...
export function RichTextEditor({
  ...
  const [theme, setTheme] = useState<ShadcnEditorTheme>(LIGHT_FALLBACK);
  ...
  const handleImageUpload = useCallback<ImagesUploadHandler>(
    async (blobInfo, progress) => {
      ...
      if (!uploadedFile) {
        throw new Error('The image upload was not completed.');
      }

      progress(90);

      const imageUrl = uploadedFile.ufsUrl ?? uploadedFile.serverData?.url;
  ...
  const chromeStyle = useMemo(() => createChromeStyle(theme), [theme]);
  ...
  const editorConfig = useMemo<TinyMceInit>(
    () => ({
      ...
      quickbars_selection_toolbar: presetConfig.quickbarsSelectionToolbar,
      quickbars_insert_toolbar: presetConfig.quickbarsInsertToolbar,
  ...
  return (
    <div className={cn('forgeui-rich-text-editor space-y-2', className)}>
